...
class Factor(models.Model):
    ...
    def algorithm_compare():
        ...
        # fname = "svmtree.dot"
        # joblib.dump(svm_clf_rbf, sys.path[0] + '\\static\\model_file\\' + fname,compress=3)
        ...
        # 测试结果的打印
        svm_answer = svm_clf_rbf.predict(x_train)
        # print(x_train)
        # print(svm_answer)
        # print(y_train)
        # print(np.mean(answer == y_train))
        ...
        # 准确率与召回率
        precision, recall, fbeta_score, support = precision_recall_fscore_support(y_test, svm_clf_rbf.predict(x_test))
        svm_x_answer = svm_clf_rbf.predict(x)
        # print svm_x_answer
        # print y
